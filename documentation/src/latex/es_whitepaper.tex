...
\begin{definition}[Corpus]
    ...
    Finally, let \texttt{V\_HASH} be the hashing algorithm used by the system.
    \begin{algorithm}
...
\begin{definition}[Signature]
    \label{signature}
    The signature $\varSigma$ uses the corpus of the \emph{Empreinte Sociométrique} to build a unique yet irreversible identifier to freely share with 
    no risk whatsoever to go back to the corpus, let alone the original data, from it.

    Algorithm \ref{algo:signature} describes how to build it from the corpus.

    Let $xt(corpus)$ be a function that transforms existing imprints in the corpus to registrars, $hash()$ the \texttt{Blake2b} hashing function 
    returning its hexadecimal string representation, and $pub(shortCode, shortenedHash)$ the function building a public imprint where the 
    $shortenedHash$ is the integer value of the imprint's \emph{hash} $\varepsilon^h$ with its first character stripped off\footnote{
        For example, for a hash value of \texttt{ab12}, you strip off the heading \texttt{a} to get \texttt{b12} and recover the latter as its integer 
        value \texttt{2834}: $shortenHash(\texttt{ab12}) = \texttt{2834}$
    }.
    \begin{algorithm}
        \KwIn{A corpus $\chi$, the token $T_k$}
        \KwOut{The signature}
        extract the set of registrars from the corpus (if any); $\mathcal{R} \gets xt(\chi)$ \\
        get existing gender $g$ and/or social security number $ss$: $\mathcal{R} \gets \{ g, ss \}$; \\
        build the prefix $P$ by concatenating the registrars, the gender and the social security \emph{hashes};$$
            P := \concat {\Big|}_{i=1}^{\norm{\mathcal{R}}} \varepsilon_i^h
        $$ \\
        set the hashed prefix: $H_P \gets hash(P)$; \\
        encrypt the hashed prefix with the token: $E_P \gets \mathcal{c}(H_P, T_k)$; \\
        finalize the prefix: $P \gets H_P[0] \concat E_P$; \\
        initialize the empty string $s^\iota$; \\
        \For{$i \gets 1$ \KwTo $\norm{\mathcal{R}}$ \KwBy $1$}{
            build its public imprint $\varepsilon_i^\pi$:$$
                \varepsilon_i^\pi \gets pub(\varepsilon^{\overline{s}}, shortenHash(\mathcal{R}_i))
            $$ \\
            $s^\iota \gets s^\iota \concat \varepsilon_i^\pi$; \\
            \If{$i < norm{\mathcal{R}}$}{
